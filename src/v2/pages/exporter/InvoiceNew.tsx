...
export default function ExporterInvoiceNew() {
  ...
  const stage2Types = useMemo(
    () => docTypes.filter((d) => d.stage === 2 && d.requirement === 'mandatory'),
    [docTypes],
  );
      ...
      <form className="space-y-6" onSubmit={submit}>
        ...
        <section className={`card-elevated space-y-3 p-6 ${step2Locked ? 'opacity-60' : ''}`}>
          ...
          <p className="text-xs text-muted-foreground">
            {step2Locked ? 'Unlocks once your submission is approved. Here is what you can prepare now.' : 'Upload these to move to funding.'}
          </p>
          {stage2Types.map((t) => (
            <DocumentUploadRow
              key={t.id}
