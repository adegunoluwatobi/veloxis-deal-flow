...
async function main() {
  ...
  // 7 storage
  {
    const path = `${expA.id}/signed-test.pdf`;
    const up = await admin.storage.from('veloxis-documents').upload(path, new Blob(['secret-A'], { type: 'application/pdf' }), { upsert: true, contentType: 'application/pdf' });
