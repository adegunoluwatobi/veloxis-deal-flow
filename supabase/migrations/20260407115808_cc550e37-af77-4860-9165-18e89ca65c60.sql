...
-- Add new status for pending rejection approval
ALTER TYPE public.deal_status ADD VALUE IF NOT EXISTS 'rejection_pending_approval' AFTER 'ready_for_final_approval';

-- Update validate_status_transition to enforce two-step rejection
...
BEGIN
  ...
  -- Deal manager: review and processing, recommend only (no direct approval/rejection)
