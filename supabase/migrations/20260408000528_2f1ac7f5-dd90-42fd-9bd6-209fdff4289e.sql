-- Migrate existing sent_to_veloxis deals to under_review
UPDATE public.deals SET status = 'under_review' WHERE status = 'sent_to_veloxis';
...
BEGIN
  ...
  IF p_user_role = 'deal_manager' THEN
    RETURN (p_current_status = 'under_review' AND p_new_status IN ('docs_requested', 'ready_for_final_approval', 'rejection_pending_approval'))
  ...
  IF p_user_role = 'super_admin' THEN
    RETURN (p_current_status = 'under_review' AND p_new_status IN ('docs_requested', 'ready_for_final_approval', 'rejection_pending_approval'))
        OR (p_current_status = 'ready_for_final_approval' AND p_new_status IN ('pending_exporter_acceptance', 'approved', 'rejected'))
...
-- Update partner RLS WITH CHECK to allow setting under_review instead of sent_to_veloxis
DROP POLICY IF EXISTS "Partners can update org deals" ON public.deals;
CREATE POLICY "Partners can update org deals"
  ...
  WITH CHECK (
    is_partner(auth.uid())
    AND status IN ('submitted', 'changes_requested', 'rejected_by_partner', 'under_review')
    AND EXISTS (
      SELECT 1 FROM exporters e
      WHERE e.id = deals.exporter_id
      AND is_partner_in_org(auth.uid(), get_partner_org_id(e.originator_id))
