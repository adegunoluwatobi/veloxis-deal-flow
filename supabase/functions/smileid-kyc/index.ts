// Initiate Smile ID KYC (Identity Verification) for an individual subject.
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.49.1";
import { corsHeaders, smileFetch, normalizeKycResult } from "../_shared/smileid.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const {
      ...
      country = "NG", id_type = "NIN", id_number,
      first_name, last_name, dob,
    } = body;
    if (!subject_type || !subject_id || !id_number || !first_name || !last_name) return json({ error: "Missing required fields" }, 400);
    ...
    const requestPayload = {
      country, id_type, id_number, first_name, last_name, dob,
      partner_params: { job_id: jobId, user_id: subject_id, job_type: 5 },
    };
    const r = await smileFetch("/identity_verification", requestPayload);
    const norm = normalizeKycResult(r.response);

    const { data: inserted, error: insErr } = await admin
      .from("verification_jobs")
      .insert({
        subject_type, subject_id, partner_organisation_id: orgId,
        job_type: "kyc", provider: "smileid",
